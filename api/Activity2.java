...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "vaishnavi") // Pass request body from file
        ...
        response.then().body("username", equalTo("vaishnavi"));
        response.then().body("firstName", equalTo("vaishu"));
        response.then().body("lastName", equalTo("Case"));
        response.then().body("email", equalTo("vaishnavi@mail.com"));
    ...
    public void deleteUser() throws IOException {
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "vaishnavi") // Add path parameter
            .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("vaishnavi"));
    }
}